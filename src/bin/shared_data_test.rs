...
fn test_signal(signal: &[f64], window: &[f64], hop: usize, fs: f64, signal_name: &str) -> Result<RustResult, String> {
    ...
    let rel_error = if signal_magnitude > 0.0 { abs_error / signal_magnitude } else { abs_error };
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Signals: {:?}", test_data.signals.keys().collect::<Vec<_>>());
    
    // Verify window matches
    let expected_window = create_hann_window(test_data.parameters.window_length);
    let window_match = test_data.parameters.window.iter()
    ...
    for result in &results {
        ...
        println!("{:<15} {:<12.2e} {}", result.signal_name, result.rust_abs_error, status);
    }
