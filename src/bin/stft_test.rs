...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create STFT object
    let stft = StandaloneSTFT::new(window, hop_length, fs, Some("onesided"), None, None, None)?;
    ...
    println!(
        "  m_num: Rust={}, Python={}",
        stft_mut.m_num(),
        test_data.stft_properties.m_num
    );
    println!(
        "  f_pts: Rust={}, Python={}",
        stft_mut.f_pts(),
        test_data.stft_properties.f_pts
    );
    println!(
        "  p_min: Rust={}, Python={}",
        stft_mut.p_min(),
        test_data.stft_properties.p_min
    );
    println!(
        "  p_max: Rust={}, Python={}",
        stft_mut.p_max(n_samples),
        test_data.stft_properties.p_max
    );
    ...
    println!(
        "Rust STFT->ISTFT vs Python STFT->ISTFT difference: {:.2e}",
        rust_diff
    );
