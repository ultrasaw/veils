...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Original impulse at index: {}", debug_data.analysis.original_impulse_idx);
    println!("  Python reconstruction error: {:.2e}", debug_data.analysis.reconstruction_error);
    ...
    println!("  m_num: {} (Python: {})", stft.m_num(), debug_data.properties.m_num);
    println!("  f_pts: {} (Python: {})", stft.f_pts(), debug_data.properties.f_pts);
    println!("  p_min: {} (Python: {})", stft.p_min(), debug_data.properties.p_min);
    println!("  p_max: {} (Python: {})", stft.p_max(debug_data.signal.len()), debug_data.properties.p_max);
    
    // Test Rust STFT forward
    let rust_stft = stft.stft(&debug_data.signal, None, None, None)?;
    println!("\nRust STFT shape: {} x {}", rust_stft[0].len(), rust_stft.len());
    println!("Python STFT shape: {} x {}", debug_data.stft_real.len(), debug_data.stft_real[0].len());
    ...
    for f in 0..debug_data.stft_real.len() {
        ...
        for t in 0..debug_data.stft_real[0].len() {
            freq_slice.push(Complex::new(debug_data.stft_real[f][t], debug_data.stft_imag[f][t]));
        }
    ...
    println!("Rust dual window sum: {:.6}", dual_win.iter().map(|x| x.re).sum::<f64>());
    println!("Rust window sum: {:.6}", debug_data.window.iter().sum::<f64>());
    println!("Rust dual window max: {:.6}", dual_win.iter().map(|x| x.re).fold(0.0f64, |a, b| a.max(b)));
    println!("Rust dual window min: {:.6}", dual_win.iter().map(|x| x.re).fold(f64::INFINITY, |a, b| a.min(b)));
    
    let rust_reconstructed_from_python = stft_mut.istft(&python_stft_native, None, None)?;
    
    println!("Rust reconstruction length: {}", rust_reconstructed_from_python.len());
    println!("Python reconstruction length: {}", debug_data.reconstructed.len());
    
    // Compare reconstructions
    let min_len = rust_reconstructed_from_python.len().min(debug_data.reconstructed.len());
    ...
    let rust_peak_idx = rust_reconstructed_from_python.iter()
        .enumerate()
    ...
    println!("\nPeak analysis:");
    println!("  Original impulse: index {}, value 1.0", debug_data.analysis.original_impulse_idx);
    println!("  Python peak: index {}, value {:.6}", debug_data.analysis.reconstructed_peak_idx, debug_data.analysis.reconstructed_peak_val);
    println!("  Rust peak: index {}, value {:.6}", rust_peak_idx, rust_peak_val);
    ...
    if max_stft_diff < 1e-10 {
        ...
        println!("❌ STFT Forward: Differences detected ({:.2e})", max_stft_diff);
    }
    
    if mean_recon_diff < 1e-10 {
        println!("✅ ISTFT Algorithm: Perfect match with Python");
    } else {
        println!("❌ ISTFT Algorithm: Differences detected ({:.2e})", mean_recon_diff);
    }
