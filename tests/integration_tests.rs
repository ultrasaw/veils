...
fn test_perfect_reconstruction_impulse() {
    ...
    let error: f64 = signal[..min_len].iter()
        .zip(reconstructed[..min_len].iter())
        .map(|(a, b)| (a - b).abs())
        .fold(0.0, f64::max);
    
    assert!(error < 1e-14, "Impulse reconstruction error {} should be < 1e-14", error);
}
...
fn test_perfect_reconstruction_sine_wave() {
    ...
    assert!(error < 1e-15, "Sine wave reconstruction error {} should be < 1e-15", error);
}
...
fn test_perfect_reconstruction_chirp() {
    ...
    assert!(error < 1e-15, "Chirp reconstruction error {} should be < 1e-15", error);
}
...
fn test_all_signals_perfect_reconstruction() {
    ...
    for test_signal in test_signals {
        ...
        assert!(!stft_result[0].is_empty(), "STFT frequency bins should not be empty");
        ...
        let error: f64 = test_signal.data[..min_len].iter()
            .zip(reconstructed[..min_len].iter())
            .map(|(a, b)| (a - b).abs())
            .fold(0.0, f64::max);
        ...
        assert!(
            ...
            test_signal.name, error, test_signal.expected_error
        );
    }
}
...
fn test_different_fft_modes() {
    ...
    for mode in &modes {
        ...
        println!("  Reconstruction error: {:.2e}", error);
        assert!(error < 1e-14, "Mode {} reconstruction error should be < 1e-14", mode);
    }
}
